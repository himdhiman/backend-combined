...
from runcode.helper import decode_data
...
class ProblemListStatusSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, obj):
        ...
        if mail_id:
            data = models.Submission.objects.filter(
                problem_Id=obj.id, created_By=mail_id
            ).order_by("-score")
            if len(data) > 0:
                data = data.first()
...
class SubmissionListSerializer(serializers.ModelSerializer):
    class Meta:
        model = models.Submission
        fields = ["language", "status", "score", "total_score", "submission_Date_Time"]
